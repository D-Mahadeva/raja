// zepto-clone/index.js
// Entry point for the Zepto clone app
...
const app = express();
const PORT = process.env.ZEPTO_PORT || 3002;
...
app.post('/api/orders', (req, res) => {
  ...
  // Create new order
  const orderId = `ZP${Date.now().toString().slice(-6)}${Math.floor(Math.random() * 1000)}`;
  const order = {
    id: orderId,
    sessionId,
    ...
    deliveryAddress,
    status: 'confirmed',
    estimatedDelivery: '8-12 minutes',
    createdAt: new Date(),
    platform: 'zepto'
  };
...
// Start the server
app.listen(PORT, () => {
  console.log(`Zepto clone running on http://localhost:${PORT}`);
});